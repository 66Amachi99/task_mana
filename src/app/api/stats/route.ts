...
export async function GET(request: NextRequest) {
  try {
    ...
    let frictionLabels: string[] = [];
    let frictionValues: number[] = [];
    if (interval === 'day') {
      const days = eachDayOfInterval({ start: startDate, end: new Date() });
      frictionLabels = days.map(d => format(d, 'dd.MM'));
      frictionValues = days.map(d => commentsRaw.filter(c => format(c.created_at, 'dd.MM') === format(d, 'dd.MM')).length);
    } else {
      const months = eachMonthOfInterval({ start: startDate, end: new Date() });
      frictionLabels = months.map(m => format(m, 'MMM', { locale: ru }));
      frictionValues = months.map(m => commentsRaw.filter(c => c.created_at.getMonth() === m.getMonth()).length);
    }

    return NextResponse.json({
      pulse: { activePosts, activeTasks, overduePosts, totalComments },
      complexity: {
          video: complexityData.filter(p => p.post_needs_video).length,
          mini_video: complexityData.filter(p => p.post_needs_mini_video_smm).length,
          cover: complexityData.filter(p => p.post_needs_cover_photo).length,
          cards: complexityData.filter(p => p.post_needs_photo_cards).length,
          gallery: complexityData.filter(p => p.post_needs_photogallery).length,
      },
      platforms: published._count,
      team: teamRaw.map(u => ({ login: u.user_login, posts: u._count.posts, tasks: u._count.assigned_tasks })),
      friction: { labels: frictionLabels, data: frictionValues },
      tags: tagsRaw.filter(t => t._count.posts > 0).map(t => ({ name: t.name, color: t.color, count: t._count.posts }))
    });
  } catch (e) {
